...
volatile int      Flag_old=1, Flag_send = 1; // 1: led on(차 없음), 0: led off(차 있음)
void ADC1_2_IRQHandler(void)
{
    ...
    if(ADC1_value <0.5){    
      ...
      LED_On();
      if(Flag_old==0){ Flag_send=1; Flag_old=1; 
      Uart2_Send_String("AT+SSEND=0,,,3\rOut\r");
      Uart1_Printf("car out\n");
